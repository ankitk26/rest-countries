...
export default function CountryCard({ country, index = 0 }: Props) {
  ...
  return (
    <Link href={`/countries/${country.cca2}`} className="block group">
      <article
        className="card-elegant rounded-elegant overflow-hidden opacity-0 animate-slide-up"
      ...
      >
        ...
        <div className="p-5">
          <h2 className="font-display text-lg md:text-xl mb-4 tracking-tight group-hover:opacity-70 transition-opacity duration-300">
            {country.name.common}
          </h2>
          ...
          <dl className="space-y-2 text-sm">
            ...
            {country.region && (
              <div className="flex gap-2">
                ...
                <dd className="font-light text-muted">{country.region}</dd>
              </div>
            )}
            {country.capital && (
              <div className="flex gap-2">
